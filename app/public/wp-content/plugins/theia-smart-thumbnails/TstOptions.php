...
add_action( 'init', 'TstOptions::init' );
...
class TstOptions {
	...
	public static function init() {
		$defaults = array(
			...
			'tst_general'   => array(
				...
				'cacheBusting'                 => true,
				'previewSizes'                 => TstOptions::get_default_preview_sizes(),
				'hidePreviewByDefault'         => false
			)
		);
		...
		foreach ( $defaults as $groupId => $groupValues ) {
			...
			if ( $groupId == 'tst_general' ) {
				if ( array_key_exists( $options['default_focal_point'], TstOptions::get_default_focal_points() ) == false ) {
				...
				if ( $options['default_focal_point'] == 'custom' ) {
					...
					$available_options                = TstOptions::get_default_focal_points();
	...
	public static function get_sizes_options_for_thumbnail( $thumbnail_id ) {
		...
		$result = array(
			'use_focus_point_mode'         => array_key_exists( 'use_focus_point_' . $thumbnail_id, $options ) ? (int) $options[ 'use_focus_point_' . $thumbnail_id ] : TstOptions::USE_FOCUS_POINT_MODE_YES,
			'crop_to_fit_mode'             => array_key_exists( 'crop_to_fit_' . $thumbnail_id, $options ) ? (int) $options[ 'crop_to_fit_' . $thumbnail_id ] : TstOptions::CROP_TO_FIT_MODE_NO,
