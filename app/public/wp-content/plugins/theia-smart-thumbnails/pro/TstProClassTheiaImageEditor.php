...
add_filter( 'tst_theia_image_editor_gd_after', 'TstProClassTheiaImageEditor::tst_theia_image_editor_gd_after', 10, 6);

class TstProClassTheiaImageEditor {
	public static function tst_theia_image_editor_gd_after( $return, $max_w, $max_h, $crop, $thumbnail_id, $that ) {
		...
		if ( Theia_Image_Editor_GD::$sizes_options['crop_to_fit_mode'] == 0 ) {
			return 'resize';
		}

		$background_color = TstMisc::hex_to_rgb( Theia_Image_Editor_GD::$sizes_options['crop_to_fit_background_color'] );

		// Match background
		if ( Theia_Image_Editor_GD::$sizes_options['crop_to_fit_mode'] == TstOptions::CROP_TO_FIT_MODE_MATCH_BACKGROUND ) {
			$pixels = array(
				array( 0, 0 ),
		...
		{
			Theia_Image_Editor_GD::$enable_crop_to_fit = true;
			$dims                     = image_resize_dimensions( $that->size['width'], $that->size['height'], $max_w, $max_h, $crop );
			Theia_Image_Editor_GD::$enable_crop_to_fit = false;
			...
			$white = imagecolorallocate( $resized, $background_color[0], $background_color[1], $background_color[2] );
