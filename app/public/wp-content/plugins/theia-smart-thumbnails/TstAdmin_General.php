...
class TstAdmin_General {
	public function echoPage() {
		TstMisc::echo_regenerate_all_thumbnails_notice();
		?>
		...
		<form method="post" action="options.php">
				...
				<tr>
					...
					<td>
						<select id="tst_general_default_focal_point" name="tst_general[default_focal_point]">
							<?php
							foreach ( TstOptions::get_default_focal_points() as $key => $value ) {
						...
						>
							...
							<div class="theiaSmartThumbnails_mediaUpload">
								<div id="focus_point_picker" class="_picker">
									<img src="<?php echo plugins_url( '/images/example-for-picker.jpg', __FILE__ ); ?>">
								</div>
							</div>
						</div>
					</td>
			...
			$disabled_name_prefix = TST_IS_PRO ? '' : 'disabled_';
			...
			<table class="form-table">
					...
					<td>
						<label>
							...
							<input type="checkbox"
							       value="true"
							       name="<?php echo $disabled_name_prefix; ?>tst_general[cacheBusting]"
								<?php echo $options['cacheBusting'] && TST_IS_PRO ? ' checked' : ''; ?>
				...
				<tr>
					...
					<td>
						...
						<input type="button"
							...
							<?php echo $disabled_if_free; ?>
							   onclick='jQuery("#tst_previewSizes").val(<?php echo json_encode( TstOptions::get_default_preview_sizes() ); ?>)'>
				...
				<tr>
					<th>
						<label for="tst_theme"><?php _e( "Hide thumbnail previews:", 'theia-smart-thumbnails' ); ?></label>
						<?php echo TstMisc::get_pro_only_notice(); ?>
					</th>
					<td>
						<label>
							<?php $checked = array_key_exists( 'hidePreviewByDefault', $options ) && $options['hidePreviewByDefault']; ?>
							<input type="hidden"
							       name="<?php echo $disabled_name_prefix; ?>tst_general[hidePreviewByDefault]"
							       value="false">
							<input type="checkbox"
							       value="true"
							       name="tst_general[hidePreviewByDefault]"
						...
						<p class="description">
							<?php _e( 'Thumbnail previews will be hidden unless the "Show Preview" button is used.', 'theia-smart-thumbnails' ); ?>
						</p>
					</td>
				</tr>
			</table>
