...
class Theia_Image_Editor_GD extends WP_Image_Editor_GD {
	public $size;
	...
	protected function _resize( $max_w, $max_h, $crop = false ) {
		...
		if ( !TstOptions::get( 'enableForUndefinedImageSizes' ) && $thumbnail_id === null ) {
			return parent::_resize( $max_w, $max_h, $crop );
		}

		// Get options.
		self::$sizes_options = TstOptions::get_sizes_options_for_thumbnail( $thumbnail_id );

		// Get post by file if necessary.
		if (!TstMisc::$last_post_id && TstOptions::get('findImageIdFromImageFile')) {
			// Get image URL.
			{
				$image_url   = $this->file;
